...
public class MoCModelWyvern<T extends MoCEntityWyvern> extends EntityModel<T> {
    ...
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(
            ResourceLocation.fromNamespaceAndPath("mocreatures", "wyvern"), "main"
    );
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer buffer,
        ...
        // Neck “plates”:
        this.neckplate3.render(poseStack, buffer, packedLight, packedOverlay, color);
        this.neck3.render(poseStack, buffer, packedLight, packedOverlay, color);

        // Torso:
        this.torso.render(poseStack, buffer, packedLight, packedOverlay, color);

        // Chested storage (if chest flag set):
        if (isChested) {
            this.storage.render(poseStack, buffer, packedLight, packedOverlay, color);
        }
        ...
        if (isSaddled) {
            ...
            this.chestbelt.render(poseStack, buffer, packedLight, packedOverlay, color);
            this.stomachbelt.render(poseStack, buffer, packedLight, packedOverlay, color);
        ...
        this.MainHead.render(poseStack, buffer, packedLight, packedOverlay, color);
        this.leftupleg.render(poseStack, buffer, packedLight, packedOverlay, color);
        this.rightupleg.render(poseStack, buffer, packedLight, packedOverlay, color);

        // Render chest armor & shoulder pads *after* legs/head so they overlay properly:
        this.ironchestarmor.render(poseStack, buffer, packedLight, packedOverlay, color);
        this.ironrightshoulderpad.render(poseStack, buffer, packedLight, packedOverlay, color);
        this.ironleftshoulderpad.render(poseStack, buffer, packedLight, packedOverlay, color);
        this.goldleftshoulder.render(poseStack, buffer, packedLight, packedOverlay, color);
        this.goldchestarmor.render(poseStack, buffer, packedLight, packedOverlay, color);
        this.goldrightshoulder.render(poseStack, buffer, packedLight, packedOverlay, color);
        this.diamondleftshoulder.render(poseStack, buffer, packedLight, packedOverlay, color);
