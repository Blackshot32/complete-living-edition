...
public class MoCModelHorse<T extends MoCEntityHorse> extends MoCModelAbstractHorse<T> {
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer buffer, int packedLight, int packedOverlay, int color) {
        if (!isGhost && vanishingInt == 0) {
            if (saddled) {
                this.headSaddle.render(poseStack, buffer, packedLight, packedOverlay, color);
                ...
                this.saddleMouthL.render(poseStack, buffer, packedLight, packedOverlay, color);
                this.saddleMouthR.render(poseStack, buffer, packedLight, packedOverlay, color);
                if (rider) {
                    this.saddleMouthLine.render(poseStack, buffer, packedLight, packedOverlay, color);
                    this.saddleMouthLineR.render(poseStack, buffer, packedLight, packedOverlay, color);
                }
            }

            if (type == 65 || type == 66 || type == 67) //mule, donkey or zonkey
            {
                this.muleEarL.render(poseStack, buffer, packedLight, packedOverlay, color);
                this.muleEarR.render(poseStack, buffer, packedLight, packedOverlay, color);
            } else {
            ...
            if (chested) {
                this.bag1.render(poseStack, buffer, packedLight, packedOverlay, color);
                this.bag2.render(poseStack, buffer, packedLight, packedOverlay, color);
            }
