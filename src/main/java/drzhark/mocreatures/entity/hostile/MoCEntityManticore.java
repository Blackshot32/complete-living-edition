...
public class MoCEntityManticore extends MoCEntityMob {
    ...
    public void setPoisoning(boolean flag) {
        if (flag && !this.level().isClientSide()) {
            MoCMessageHandler.INSTANCE.send(MoCPacketDistributor.NEAR.with( () -> new MoCPacketDistributor.TargetPoint(this.getX(), this.getY(), this.getZ(), 64, this.level().dimension())), new MoCMessageAnimation(this.getId(), 0));
        }
