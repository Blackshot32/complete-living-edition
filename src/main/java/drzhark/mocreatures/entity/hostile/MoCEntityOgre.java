...
public class MoCEntityOgre extends MoCEntityMob {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(4, new MoCEntityOgre.AIOgreTarget<>(this, net.minecraft.world.entity.npc.AbstractVillager.class, true));
    }
    ...
    public void aiStep() {
        if (!this.level().isClientSide()) {
            if (this.smashCounter > 0 && ++this.smashCounter > 10) {
                ...
                MoCMessageHandler.INSTANCE.send(MoCPacketDistributor.NEAR.with( () -> new MoCPacketDistributor.TargetPoint(this.getX(), this.getY(), this.getZ(), 64, this.level().dimension())), new MoCMessageExplode(this.getId()));
            }
    ...
    protected void startDestroyBlast() {
        this.smashCounter = 1;
        MoCMessageHandler.INSTANCE.send(MoCPacketDistributor.NEAR.with( () -> new MoCPacketDistributor.TargetPoint(this.getX(), this.getY(), this.getZ(), 64, this.level().dimension())), new MoCMessageAnimation(this.getId(), 3));
    }
    ...
    protected void startArmSwingAttack() {
        if (!this.level().isClientSide()) {
            ...
            if (leftArmW) {
                this.armToAnimate = 1;
                MoCMessageHandler.INSTANCE.send(MoCPacketDistributor.NEAR.with( () -> new MoCPacketDistributor.TargetPoint(this.getX(), this.getY(), this.getZ(), 64, this.level().dimension())), new MoCMessageAnimation(this.getId(), 1));
            } else {
                this.armToAnimate = 2;
                MoCMessageHandler.INSTANCE.send(MoCPacketDistributor.NEAR.with( () -> new MoCPacketDistributor.TargetPoint(this.getX(), this.getY(), this.getZ(), 64, this.level().dimension())), new MoCMessageAnimation(this.getId(), 2));
            }
        }
    }
